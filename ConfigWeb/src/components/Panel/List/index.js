...
function List(props) {
  ...
  const _addItem = async (questionTypeId, _newItemIndex = newItemIndex) => {
    let maxItemno;
    try {
      maxItemno = list[list.length - 1].itemno + 1;
    } catch (error) {
      maxItemno = 1;
    }
    const res = await props.addItem({
      questionTypeId,
      newItemIndex: maxItemno,
    });
    //重新排序
    if (_newItemIndex) {
      const otherList = list.filter(
        (row) => row.id !== res.payload.data.data.id
      );
      const filterResIdList = list.filter(
        (row) => row.id !== res.payload.data.data.id
      );
      const sortList = [];
      otherList.map((row, i) => {
        if (i === _newItemIndex) {
          sortList.push(filterResIdList[0]);
        }
        sortList.push(row);
      });
      setRenderList(sortList);
      // 重新排序
      reorderList(sortList);
